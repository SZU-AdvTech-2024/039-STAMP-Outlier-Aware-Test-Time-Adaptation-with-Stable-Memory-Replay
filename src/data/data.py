...
def get_transform(dataset_name, adaptation, num_augment=1):
    ...
    elif adaptation in {"stamp", "stamp_UA"}:
        original_transform = get_transform(dataset_name, None) # Kyle:原始变换只有To_tensor
        transform_list = [original_transform]
        transform_one = transforms.Compose([original_transform, get_stamp_transforms(dataset_name)]) # Kyle:原始变换加上一系列变换 作为一个TTAug
        for i in range(num_augment):
            transform_list.append(transform_one) # Kyle:把15种TTAug变换加成表(每次变换有随机性)
        transform = NCropsTransform(transform_list) # Kyle:得到1+15的变换表后 传入NC实例化一个对象 传入x后会相应输出16种变换的tsfm(x)
    else:
...
def load_cifar10_c(root, corruption, level=5, batch_size=64, workers=4, transforms=None, ckpt=None):
    ...
    xtest, ytest = load_cifar10c(n_examples=10000, severity=level, data_dir=root, shuffle=False,
                                 corruptions=[corruption]) # Kyle:这里加载了stamp_lib/datasets/CIFAR-10-C中的npy文件 只是得到两个list
    teset = CustomCifarC_Dataset((xtest, ytest), transform=transforms) # Kyle:这里传入([xtest],[ytest]) 才构建了dataset

    ckpt_dir = os.path.join(ckpt, 'cifar10_c')
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
    ckpt_path = os.path.join(ckpt_dir, 'list.pickle') # Kyle:这里在产生序列化文件
    if not os.path.exists(ckpt_path):
        idx = torch.randperm(len(teset))
        idx = [i.item() for i in idx]
        with open(ckpt_path, 'wb') as f:
        ...
        with open(ckpt_path, 'rb') as f:
            idx = pickle.load(f)
    teset = torch.utils.data.Subset(teset, idx) # Kyle:这里传入固定的乱序idx 借助Subset用于打乱数据集
...
def load_cifar10(root, batch_size=64, workers=4, split="train", transforms=None):
    ...
    if split == 'train': # cifar-10-batches-py 是 torchvision.datasets.CIFAR10 默认使用的文件夹名称 首次使用下载到数据集文件夹了
...
def load_ood_dataset_test(root, ID_dataset_name, OOD_dataset_name, num_OOD_samples, batch_size=64, workers=4,
    ...
    transforms_ood = transforms.Compose([transforms.Resize((size, size)), transform]) # Kyle:注意ood数据集就不用花里胡哨的transform了
    ...
    dataset = ConcatDataset([ID_dataset, OOD_dataset]) #Kyle:注意这里ID和OOD的数据集拼接没有Shuffle了 就是先ID然后ODD
    # Kyle
    print(f"len(dataset) = len({ID_dataset_name}) {len(ID_dataset)} + len({OOD_dataset_name}) {len(OOD_dataset)} ---- {len(dataset)}")
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=workers, drop_last=False)
    return dataset, dataloader
