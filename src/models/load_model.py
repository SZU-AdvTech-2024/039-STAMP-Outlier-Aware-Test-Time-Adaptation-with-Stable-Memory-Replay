...
def load_model(model_name, checkpoint_dir=None, domain=None):
    ...
    # 增加N+1分类头策略
    elif model_name == 'ResNet18_11':
        model = resnet18_cifar(num_classes=11)
        model.num_classes=11
        # -----------------训练用 无权重导入 ---------------
        if checkpoint_dir is not None:
            checkpoint_path = "/home/ubuntu/stamp_ln/pre-train/models/ResNet18_11_UA.pt" #os.path.join(checkpoint_dir, 'ResNet18_10.pt')
            if not os.path.exists(checkpoint_path):
                raise ValueError('No checkpoint found at {}'.format(checkpoint_path))
            model.load_state_dict(torch.load(checkpoint_path, weights_only=True))
