...
def get_accuracy(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            samples += len(data[0])
            imgs, labels = data[0], data[1].cuda() # Kyle: data是一个俩元素的list [[tensor1:64×3×32×32, tensor2, ... , tensor16],[tensor:一个batch的label]]
            ...
            if isinstance(output, tuple):
                ...
                print("===========ugd_tur detected ==============")
                
                
                logits = torch.ones(len(output), 10)  
                
                for i in range(len(output)):
                    class_label = output[i]
                    if class_label < 10:
                        
                        logits[i, class_label] = 5.0
                    else:
                ...
                entropy = torch.distributions.Categorical(probs=softmax_logits).entropy()
        ...
        """
        num_class 对于 stamp_UA应当是11 但label本身就只有0-9 以及 255 不用改了
        """
...
def get_args():
    ...
    parser.add_argument('--cfg', default=None, type=str, help='path to config file') # 这个在运行Shell脚本时传入
    ...
    parser.add_argument('--output_dir', default=None, type=str, help='path to output_dir file') # 这个在运行Shell脚本时传入
