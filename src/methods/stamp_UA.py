...
def entropy(p):
    p_known = p[:, :10]  
    p_known = p_known / p_known.sum(dim=1, keepdim=True)  
    return -torch.sum(p_known * torch.log(p_known + 1e-5), dim=1)
...
class STAMP_UA(nn.Module):
    ...
    def collect_params(model):
        ...
        for nm, m in model.named_modules():
            ...
            if isinstance(m, (nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)):
                print("--------- BN param accepted --------")
                for np, p in m.named_parameters():
                    if np in ['weight', 'bias']:  # weight is scale, bias is shift
                        params.append(p)
                        names.append(f"{nm}.{np}")

            # Include final linear layer parameters
            # if hasattr(model, 'fc'):
            if isinstance(m, nn.Linear):
                print("--------- linea param accepted --------")
                for np, p in m.named_parameters():
                    if np in ['weight', 'bias']:  # weight is scale, bias is shift
                        params.append(p)
                        names.append(f"{nm}.{np}")
    ...
    def forward(self, x):
        output = self.update_memory(x) # Kyle:([64,10], [64])
        if len(self.mem) != 0:
            self.adapt()
        return output
    ...
    def update_memory(self, x):
        ...
        if self.num_class == 1000:
            ...
            pred_origin = output_origin.softmax(dim=1).max(dim=1)[1]  # [batch_size]
            pred_mean = output.max(dim=1)[1]  # [batch_size]

            
            agree_mask = pred_origin == pred_mean  
            known_mask = pred_origin < 10  
            final_mask = agree_mask & known_mask  

            
            x_append = x_origin[final_mask]
            class_ids = pred_mean[final_mask]

           
            self.mem.append(x_append, class_ids)
