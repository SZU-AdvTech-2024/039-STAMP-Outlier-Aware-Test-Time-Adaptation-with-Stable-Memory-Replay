...
from .stamp_UA import STAMP_UA
from .ugd_tur import UGDTUR
...
def setup_stamp(base_model, cfg):
    params, param_names = STAMP.collect_params(base_model)  # Kyle:params只收集了ResNet的Norm层的仿射参数
    # optimizer = setup_optimizer(params, cfg)
    base_optimizer = optim.SGD  # Kyle:这里只是把SDG定义为基优化器类 在SAM中传入params进一步才实例化
...
def setup_stamp_UA(base_model, cfg):
    params, param_names = STAMP_UA.collect_params(base_model)  # stamp_UA 除了BN参数 最后一层线性参数也要微调
    # optimizer = setup_optimizer(params, cfg)
    base_optimizer = optim.SGD  # Kyle:这里只是把SDG定义为基优化器类 在SAM中传入params进一步才实例化
    print(f"=============={cfg.OPTIM.LR}===========")
    optimizer = SAM(params, base_optimizer, lr=cfg.OPTIM.LR, rho=0.05)
    model = STAMP_UA(base_model, optimizer, cfg.STAMP.ALPHA, cfg.num_classes)
    return model

def setup_ugd_tur(base_model, cfg):
    # params, param_names = STAMP_UA.collect_params(base_model)  # stamp_UA 除了BN参数 最后一层线性参数也要微调
    # optimizer = setup_optimizer(params, cfg)
    # base_optimizer = optim.SGD  # Kyle:这里只是把SDG定义为基优化器类 在SAM中传入params进一步才实例化
    print(f"=============={cfg.OPTIM.LR}===========")
    # optimizer = SAM(params, base_optimizer, lr=cfg.OPTIM.LR, rho=0.05)
    model = UGDTUR(base_model,  cfg)
    return model
...
def setup_model(cfg):
    ...
    elif cfg.MODEL.ADAPTATION == 'stamp_UA':
        # cfg.num_classes = get_num_classes(dataset_name=cfg.CORRUPTION.ID_DATASET)
        cfg.num_classes = 11 # 直接把未知类当作一类进行处理
        base_model = load_model(model_name=cfg.MODEL.ARCH, checkpoint_dir="/home/ubuntu/stamp_ln/pre-train/models/ResNet18_11_UA.pt",
                            domain=cfg.CORRUPTION.SOURCE_DOMAIN)
        base_model = base_model.cuda()
        model = setup_stamp_UA(base_model, cfg)

    elif cfg.MODEL.ADAPTATION == 'ugd_tur':
        # cfg.num_classes = get_num_classes(dataset_name=cfg.CORRUPTION.ID_DATASET)
        cfg.num_classes = 11 # 直接把未知类当作一类进行处理
        base_model = load_model(model_name=cfg.MODEL.ARCH, checkpoint_dir="/home/ubuntu/stamp_ln/pre-train/models/ResNet18_11_UA.pt",
                            domain=cfg.CORRUPTION.SOURCE_DOMAIN)
        base_model = base_model.cuda()
        model = setup_ugd_tur(base_model, cfg)
    else:
