...
class STAMP(nn.Module):
    ...
    def update_memory(self, x):
        ...
        if self.num_class == 1000:
            ...
            output = torch.stack(outputs, dim=0) 
            output = torch.mean(output, dim=0) 
            filter_ids_0 = torch.where(output_origin.softmax(dim=1).max(dim=1)[1] == output.max(dim=1)[1])
            entropys = entropy(output)[filter_ids_0]
            # Kyle:
            # print("print(entropys.shape):")
    ...
    def adapt(self):
        ...
        # data = x_origin
        if len(data) > 0:
            output_1 = self.model(data) # Kyle:torch.Size([17, 10])
            entropys = softmax_entropy(output_1) # Kyle:torch.Size([17]) 对每个RBM中样本的预测分布求出一个熵
            # coeff = 1 / (torch.exp(entropys.clone().detach() - self.margin))
            inv_entropy = 1 / torch.exp(entropys) 
            coeff = inv_entropy / inv_entropy.sum() * 64 # 这里直接×64 没有顾忌开始RBM没填满？？(事实上过5轮才填满64可能没啥影响)
            entropys = entropys.mul(coeff)
            loss = entropys.mean() # 
            loss.backward()
            self.optimizer.first_step(zero_grad=True) # 参数沿梯度方向攀升(扰动到邻域最大值) w+e(w) 然后清零梯度信息 
            ...
            entropys = entropys.mul(coeff)
            loss = entropys.mean()
            loss.backward() # Kyle:在新位置再算一次loss 再求一次梯度(真正要用的梯度)
            self.optimizer.second_step(zero_grad=True) # Kyle:利用self.state[p]["old_p"]返回原位置 然后做真正的迭代
            self.scheduler.step()
