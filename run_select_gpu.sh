#------------------ All available datasets combinations -----------------#
#
#DATASETS=("cifar10c_8_2" "cifar100c_80_20" 
#           "imagenet_c_places" "imagenet_c_text"
#           "cifar10_c_noise" "cifar10_c_svhn" "cifar10_c_lsun" "cifar10_c_tiny" 
#           "cifar100_c_noise" "cifar100_c_svhn" "cifar100_c_lsun" "cifar100_c_tiny")
#
#------------------------------------------------------------------------#
...
METHODS=(
         #"source" "norm_test" "cotta" "eata" "tent" "sar" "rotta" "owttt" "sotta" "stamp"
         # "stamp"
         "stamp_UA"
         # "ugd_tur"
         #"tent"
         #"cotta"
         #"source"
         #"norm_test" "cotta" "eata" "tent" "sar" "rotta" "owttt" "sotta"
        )

AVAILABLE_GPUS=(7 8 9 )
gpu_index=0 # 可用gpu数组中的索引

test-baseline(){
  DATASET=$1
  METHOD=$2
  GPU_id=${AVAILABLE_GPUS[$gpu_index]} # 从可用 GPU 数组中获取当前 GPU ID
  ...
  if [ "$METHOD" == "tent" ]; then
    ...
    CUDA_VISIBLE_DEVICES="$GPU_id" python TTA-evaluation.py --cfg cfgs/Online_TTA_os/${DATASET}/${METHOD}.yaml --output_dir "${output_dir}" \
      --OPTIM_LR "$lr" &
    ...
    CUDA_VISIBLE_DEVICES="$GPU_id" python TTA-evaluation.py --cfg cfgs/Online_TTA_os/${DATASET}/${METHOD}.yaml --output_dir "${output_dir}" \
      --OPTIM_LR "$lr" --STAMP_ALPHA "$alpha" # 把后台运行符先注释一下 不然无法ctrl + c 终止 这里只跑单个GPU
  elif [ "$METHOD" == "stamp_UA" ] || [ "$METHOD" == "ugd_tur" ]; then
    if echo "$DATASET" | grep -q "cifar100_c";then
      lr=0.05
      alpha=0.9
    elif echo "$DATASET" | grep -q "cifar10_c";then
